...
function ScheduledMeeting() {
     ...
     * @param {string} type 
     * @returns {Array} Filtered meeting list
     */
    const filterMeetingList = (type) => {
        const currentTime = Date.now(); // Use Date.now() for numeric timestamp
        if (type === 'upcoming') {
            return meetingList.filter(item => item.formatedTimeStamp >= currentTime);
        } else {
            return meetingList.filter(item => item.formatedTimeStamp < currentTime);
        }
    };
